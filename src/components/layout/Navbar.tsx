            ...
            if (link.children) {
              return (
                <li key={link.href} className="group relative">
                  ...
                  {/* Dropdown */}
                  <div className="invisible absolute left-1/2 top-full z-50 w-[20rem] -translate-x-1/2 pt-3 opacity-0 transition-all duration-200 group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100">
                      ...
                      <AnimatePresence initial={false}>
                        {subOpen && (
                          ...
                          >
                            {link.children.map((child) => {
                              const slug = child.href.split("/").pop() ?? "";
                              const Icon = expertiseBySlug.get(slug)?.icon;
                              return (
                                <li key={child.href}>
                                  <Link
                                    href={child.href}
